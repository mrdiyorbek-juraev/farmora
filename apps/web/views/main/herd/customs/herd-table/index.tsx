...
const allColumnDefs: Record<string, ColDef<CattleRow>> = {
  tag_number: {
    ...
    cellRenderer: (params: { value: string | null; data?: CattleRow }) => {
      ...
      return (
        <span className="font-medium tabular-nums">{params.value}</span>
      );
    },
  },
  name: {
...
export function HerdTable({
  ...
  const columnDefs = useMemo<ColDef<CattleRow>[]>(() => {
    ...
    // Forward the `onAddNew` callback into the Tag column's header
    // component via AG Grid's `headerComponentParams`. We do it here
    // (not in `allColumnDefs`) so the closure always references the
    // latest prop, even if HerdView rebinds it across renders.
    const withHeader = visible.map((def) => {
      ...
      return {
        ...def,
        headerComponentParams: { onAddNew },
      };
    });

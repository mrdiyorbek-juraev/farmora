...
import { ArrowRight, Mars, SearchX, Sprout, Venus } from "lucide-react";
...
export function GlobalSearch() {
  const { search, setModal } = useGlobalModal();
  ...
  // Reset query alongside the close so re-opening starts blank and
  // doesn't flash the previous result list. Routed through every
  // close-path (radix onOpenChange covers ESC + outside-click + the
  // dialog's own close, plus explicit handler calls below).
  const handleOpenChange = (next: boolean) => {
    setModal({ search: { open: next } });
    if (!next) {
      setQuery("");
    }
  };

  // ⌘K toggles even while a field is focused; "/" only when not typing,
  // so it never hijacks a slash inside an input.
  useHotkeys(
    "mod+k",
    (event) => {
  ...
  return (
    <CommandDialog
      className="w-[calc(100vw-2rem)] max-w-3xl sm:w-full top-30"
          ...
          {(() => {
            ...
            if (rows.length === 0) {
              ...
              return debouncedQuery ? (
                ...
                <NoAnimalsState onClose={() => handleOpenChange(false)} />
              );
            }
...
function ErrorState({ message }: { message: string }) {
  return <div className="p-4 text-destructive text-sm">{message}</div>;
}

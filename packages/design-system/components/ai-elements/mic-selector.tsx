...
export const MicSelectorList = ({
  ...
  return (
    <CommandList {...props}>
      <CommandGroup>{children(data)}</CommandGroup>
    </CommandList>
  );
};
...
export type MicSelectorItemProps = Omit<
...
export const MicSelectorItem = ({
  ...
  const { onValueChange, onOpenChange, value } = useContext(MicSelectorContext);

  const handleSelect = useCallback(() => {
    onValueChange?.(deviceId);
    onOpenChange?.(false);
  }, [deviceId, onValueChange, onOpenChange]);

  return (
    <CommandItem
      data-checked={deviceId === value ? true : undefined}
